...
<template>
  <Teleport to="body">
    <transition name="drawer-backdrop">
      <div v-if="open" class="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" @click="close" @keydown.escape="close"></div>
    </transition>
    <transition name="drawer-panel">
      <div v-if="open" class="fixed top-0 right-0 z-50 h-full w-full max-w-md bg-slate-950 border-l border-slate-800 shadow-2xl flex flex-col" role="dialog" aria-modal="true" aria-label="Shopping cart">
        ...
        <div v-if="cart.length > 0" class="border-t border-slate-800 px-5 py-4 space-y-3">
          ...
          <router-link to="/checkout" @click="close" class="block w-full text-center bg-cyan-600 text-white py-3 rounded-md font-semibold hover:bg-cyan-500 active:scale-[0.98] transition-all duration-150 focus-visible:outline-2 focus-visible:outline-cyan-400">
            View Cart & Checkout
          </router-link>
