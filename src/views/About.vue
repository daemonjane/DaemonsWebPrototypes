...
<template>
  <div class="max-w-4xl mx-auto px-4 py-12 relative">
    ...
    <!-- Stats row -->
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-10 py-6 border-y border-surface-700">
    ...
    <!-- Mission -->
    <div class="bg-surface-900 p-6 sm:p-8 rounded-xl border border-surface-700 space-y-4 mb-8">
      <h2 class="text-xl font-semibold font-display text-surface-50">Our Mission</h2>
    ...
    <div class="grid sm:grid-cols-2 gap-4 mb-8">
      ...
      >
        <div class="flex items-center gap-3 mb-2">
          ...
          <h3 class="font-semibold text-surface-50">{{ v.title }}</h3>
        </div>
        <div v-if="expandedValue === v.id" class="mt-2 text-sm text-surface-400 leading-relaxed animate-[fadeIn_0.2s_ease]">
          {{ v.detail }}
        </div>
        <p v-else class="text-xs text-surface-500 mt-1">Click to expand →</p>
      </button>
    </div>

    <!-- Tech Stack -->
    <div class="bg-surface-900 rounded-xl border border-surface-700 p-6 mb-8">
      <h2 class="text-xl font-semibold font-display text-surface-50 mb-4">Tech Stack</h2>
      <div class="space-y-3">
    ...
    <div class="bg-surface-900 rounded-xl border border-surface-700 mb-8">
      <button
        @click="timelineOpen = !timelineOpen"
        class="w-full flex items-center justify-between p-6 text-left"
        :aria-expanded="timelineOpen"
      >
        <h2 class="text-xl font-semibold font-display text-surface-50">Milestones</h2>
        <svg class="w-5 h-5 text-surface-400 transition-transform duration-200" :class="{ 'rotate-180': timelineOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
