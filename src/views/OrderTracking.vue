...
<template>
  <div class="max-w-2xl mx-auto px-4 py-12">
    ...
    <h1 class="text-3xl font-bold text-surface-50 font-display mb-2">Order Tracking</h1>
    <p class="text-surface-400 mb-8 text-sm">Enter your order number to view the current status and delivery timeline.</p>
    ...
    <div v-else-if="error" class="bg-danger-500/10 border border-danger-500/50 rounded-xl p-6 text-center">
      <p class="text-danger-400">{{ error }}</p>
    </div>

    <div v-else-if="searched && tracking && !tracking.tracking_number" class="bg-surface-900 rounded-xl border border-surface-700 p-6 text-center">
      <p class="text-surface-500">No tracking information available for this order yet.</p>
    </div>

    <div v-else-if="tracking && tracking.tracking_number" class="bg-surface-900 rounded-xl border border-surface-700 p-6 sm:p-8">
      <div class="flex items-center justify-between mb-6">
        <div>
          <p class="text-xs text-surface-500 font-mono uppercase tracking-wider">Order #{{ tracking.order_id }}</p>
          <p class="text-surface-50 font-mono font-bold text-lg mt-1">{{ tracking.carrier }} {{ tracking.tracking_number }}</p>
          <p v-if="tracking.estimated_delivery" class="text-xs text-surface-500 mt-1">
      ...
      <div class="relative" v-if="tracking.history && tracking.history.length">
        <div v-for="(step, i) in tracking.history" :key="i" class="flex gap-4 pb-8 last:pb-0 relative">
          ...
          <div class="pt-0.5">
            <p class="text-surface-50 font-medium text-sm"
               :class="{ 'text-surface-500': !isPast(i, tracking.history) && i !== currentIndex(tracking.history) }">
              {{ step.status }}
            </p>
            <p class="text-xs text-surface-500 mt-0.5 font-mono">{{ formatDate(step.timestamp) }}</p>
            <p v-if="step.location" class="text-xs text-surface-600 mt-0.5">{{ step.location }}</p>
            <p v-if="step.note" class="text-xs text-surface-600 mt-0.5">{{ step.note }}</p>
          </div>
        </div>
      </div>
      <div v-else class="text-center py-6 text-surface-500 text-sm">
        No tracking history yet.
      </div>
    </div>

    <div v-else-if="searched && !tracking" class="text-center py-12">
      <p class="text-surface-500">Enter an order number to track.</p>
    </div>
  </div>
</template>
